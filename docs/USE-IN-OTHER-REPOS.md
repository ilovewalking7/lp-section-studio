...
- **レジストリURL（ベース）**: `https://lp-section-studio.pages.dev/r/`
- **一覧（全 id/type/category）**: `https://lp-section-studio.pages.dev/r/index.json`
...
```bash
# 全件
curl -s https://lp-section-studio.pages.dev/r/index.json | jq '.items[] | {name, type, category}'

# カテゴリで絞る（例: ダークテック）
curl -s https://lp-section-studio.pages.dev/r/index.json \
...
npx shadcn@latest add https://lp-section-studio.pages.dev/r/<id>.json

# 既存の components/ui/* や lib/utils.ts を上書きしてよければ
npx shadcn@latest add --overwrite https://lp-section-studio.pages.dev/r/<id>.json
```
...
curl -s https://lp-section-studio.pages.dev/r/<id>.json | jq -r '.files[0].content'
```
...
2) 候補を見る: curl -s https://lp-section-studio.pages.dev/r/index.json | jq -r '.items[].name'
3) `npx shadcn@latest add https://lp-section-studio.pages.dev/r/<id>.json` で取り込む
4) 適当なページで import して表示し、ビルドが通るか確認して
※ サンドボックスから curl が 403（Host not in allowlist）なら、この環境の egress 許可に
  lp-section-studio.pages.dev を追加してから再実行。
```

### 素のHTML/CSS/JS リポジトリ用
```
私の公開レジストリのコンポーネントを、このバニラ(HTML/CSS/JS)サイトに移植したい。
1) 一覧: curl -s https://lp-section-studio.pages.dev/r/index.json | jq -r '.items[] | "\(.name)\t\(.category)"'
2) ソース取得: curl -s https://lp-section-studio.pages.dev/r/<id>.json | jq -r '.files[0].content'
...
- **増えたら自動配信**: lp-section-studio の `src/registry/demos/` にコンポーネントを足して push すれば、デプロイ時に `/r/*` も自動更新される。
